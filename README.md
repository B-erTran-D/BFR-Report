...
| Étape | Contenu |
|---|---|
| **Client & machine(s)** | Client par **autocomplétion** (3 lettres), contact sur site, fonction, téléphone du client, e-mail, adresse, lieu d'intervention, logo du client (facultatif), machine principale (désignation, modèle, n° de série) et bouton **➕ Ajouter une autre machine** pour les interventions multi-machines, collègues accompagnants sur site (**➕ Ajouter un collègue**), objet de la demande, N° de rapport `AAMMJJ-NN` généré automatiquement |
| **Chrono & Multi-jours** | Bouton **Démarrer l'intervention** : l'heure de début est horodatée. Pauses et fin d'intervention ; la durée sur site est calculée pauses déduites. Bouton **✏️ Ajuster** pour activer le relevé **multi-jours** (saisie par journée des heures, pauses et activités avec calcul automatique du total cumulé) |
| **Ajouter un évènement** | Assistant en 4 étapes : **1. domaine** (mécanique / électrique / automatisme) et **machine concernée** (sélecteur dédié en intervention multi-machines) → **2. annotation** écrite ou dictée, avec relecture vocale → **3. photo** prise au téléphone puis **annotée au doigt** (flèche, cercle, crayon, texte, 5 couleurs) → **4. catégorie** (🛑 Sécurité, ⚠️ Urgent, 🔺 Priorité haute, 🔽 Priorité basse, ℹ️ Informatif). L'évènement est **modifiable à tout moment** : on appuie dessus pour rouvrir l'assistant |
| **Compte rendu** | Résumé / synthèse, travaux réalisés, travaux à prévoir (base du devis), note interne |
| **Pièces de rechange** | Bouton **➕ Ajouter pièce de rechange** : référence, quantité, dénomination des pièces neuves échangées ou laissées dans le stock client. Tableau dédié sur le rapport ; la signature du client vaut pour acceptation du devis final |
